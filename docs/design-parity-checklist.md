...
| `web/src/components/ModelCard/` is a shared rendering owner reused by the admin Live Preview's detail-mode preview, alongside `DatasetCard`/`DatasetDetail`/`InferenceForm`/`InferenceResult` above; this component was a real, previously-undocumented dependency until the M48-04 acceptance pass. | Direct re-read of `web/src/pages/admin/DatasetAdminPage.tsx` (M48-04 issue analysis/operational-state grounding) | `web/src/components/ModelCard/ModelCard.tsx`; `web/src/pages/admin/DatasetAdminPage.tsx` | Confirmed via direct re-read: `ModelCard.tsx` is imported at line 16 and rendered via `projectModelCardPreview` at line 2350 inside Live Preview's detail mode. Added to this section as a real Live Preview dependency (M48-04 acceptance pass); no code change required. | required | M48-04 (documentation only) |
...
M48-04 (Dataset Admin/Live Preview layout and interaction parity pass)
re-confirmed the Dataset Admin, Live Preview, and Shared Public Components
rows below by direct re-read of `DatasetAdminPage.tsx`, `DatasetAdminPage.test.tsx`,
`livePreviewProjection.ts`, `livePreviewProjection.test.ts`, `InferenceForm.tsx`,
and `DatasetDetailHeader.tsx` against their `design/screens/dataset-admin/` and
`design/screens/dataset-detail/` references and against
`contracts/dataset-public-profile.schema.json`/`contracts/predict-view-customization.schema.json`.
No structural or interaction gap was found, and no code file was modified by
M48-04; its material repository change is this documentation update itself.
Three residual items are recorded below (in the Dataset Admin and Shared
Public Components sections): a schema-confirmed non-gap finding for the Home
card icon selector grid, a newly-documented Live Preview shared-component
dependency (`ModelCard.tsx`), and a disambiguation of the formal issue's own
candidate-file naming conflation between `PredictViewList.tsx` and the actual
predict-view-customization builder.
...
| Desktop rail-collapse toggle (clicking the always-visible nav toggle at desktop width collapses the rail to zero width). | implemented (now test-covered; M48-05) | `web/src/layouts/PublicShell.tsx` | `design/screens/home/desktop/script.js`, `design/screens/dataset-detail/desktop/script.js` | Direct re-read confirmed the mechanism (`navOpen` state, `public-shell--nav-open` class, `aria-expanded`, the desktop `@media (min-width: 768px)` CSS rule) was already fully implemented but had zero dedicated test coverage. `PublicShell.test.tsx` `it("collapses the rail when the desktop nav toggle is clicked")` closes this gap (M48-05); no production code changed. Closes the M41-02 "Public desktop rail toggle"/"Dataset Detail desktop rail toggle" rows' remaining test-coverage gap. | None; documentation and test coverage both closed by M48-05. |
...
| Run status filter (dropdown filters the runs table by `run.status`, already admin-API-sourced). | implemented (now test-covered; M48-05) | `web/src/pages/admin/DashboardPage.tsx` | `design/screens/dataset-admin-home/desktop/script.js` | Direct re-read confirmed `statusFilter` state and the "Run status" select already filter `filteredRuns` by real API-sourced status values, but zero existing test exercised the select. `DashboardPage.test.tsx` `it("filters the runs table by Run status when a specific status is selected")` closes this gap (M48-05); no production code changed. Closes the M41-02 "Dashboard status filter" row's remaining test-coverage gap. | None; documentation and test coverage both closed by M48-05. |
...
| Home card icon selector grid renders 18 options (3 legacy back-compat values: `telecom`, `bank`, `generic` + 15 curated icons). | implemented (confirmed non-gap; M48-04 acceptance pass) | `web/src/pages/admin/DatasetAdminPage.tsx` (`HOME_CARD_ICON_OPTIONS`, lines 51-69) | `design/screens/dataset-admin/visual-spec.md` ("15 controlled icon options") | Direct re-read confirmed `HOME_CARD_ICON_OPTIONS` and `contracts/dataset-public-profile.schema.json`'s `home_card.icon` enum list the exact same 18 values in the same order; the schema's own description states the 3 legacy values are retained for backward compatibility with already-published data. The design spec's "15 controlled icon options" phrasing describes only the curated bank Atlas added, not the schema's full selectable set — no divergence exists once both sources are read together. | None; recorded to prevent a future issue from misreading the 18-option grid as an unresolved parity gap. |
| `docs/milestones.md`'s M48 formal-issue candidate file list names `web/src/components/PredictViewList/PredictViewList.tsx` as a "predict-view customization" edit target for this route. | implemented (disambiguation; no change needed) | `web/src/components/PredictViewList/PredictViewList.tsx` | — | Direct re-grep confirmed `PredictViewList.tsx` is imported and rendered only by the public `web/src/pages/DatasetPage.tsx` (lines 14, 521); it has zero relationship to Dataset Admin or Live Preview. The actual predict-view customization concept (the field_hints/groups ordering builder governed by `contracts/predict-view-customization.schema.json`) is implemented directly inside `DatasetAdminPage.tsx` and consumed by `InferenceForm.tsx`'s `previewMode` prop, not through `PredictViewList` at all (M48-04 acceptance pass). | None; recorded to prevent a future Dataset Admin/Live Preview issue from treating `PredictViewList.tsx` as an edit target again. |
...
- `App.test.tsx` carries two admin-disabled denial assertions covering two independent entry points into the shared `<Route path="/admin/*" element={null} />` fallback: `it("does not render admin shell or admin navigation for direct admin URLs when admin is disabled")` (nested `/admin/dataset-admin`) and `it("does not render admin shell or admin navigation for the bare /admin route when admin is disabled")` (bare `/admin`, added M48-06). Settings and Help are not each covered by their own dedicated denial test; they share the identical fallback mechanism these two tests exercise, and `it("keeps public routes available when admin is disabled")` separately confirms public routing is unaffected. This proportionate-coverage scoping is a deliberate M48-06 judgment (not a permanent limit -- a future issue could still add per-route denial tests for Settings/Help if warranted), not a gap in the underlying denial mechanism itself, which is identical across all four admin routes.
- All AdminShell, Admin Dashboard, Dataset Admin, Settings, and Help entries above were re-confirmed against the M46/M47 boundary via the two denial tests and the public-routes-available test named above, which remain passing-shaped assertions in the current test file.
