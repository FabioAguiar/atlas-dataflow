...
def build_release_candidate_handoff_readiness(
    ...
    blocking_reasons = [
        f"{result['role']}: {result['reason']}" for result in role_results if _is_blocking(result)
    ]
...
# Project Spec S0216: the v2 (multiclass) INTERNAL Atlas-native fixed-
# configuration training-record profile -- still an internal M24 candidate
# (never coerced to the external manual_governed_input path), but its own
# real declared contract_version/training_metrics version must be carried
# forward into candidate input verbatim rather than the fixed v1 governance
# constants below.
_TRAINING_PARAMETER_RECORD_INTERNAL_VERSION_V2 = "training-parameter-record.v2"
...
_TRAINING_METRICS_INTERNAL_VERSION_V2 = "training-metrics.v2"
...
_ANALYTICAL_VISUALIZATIONS_INTERNAL_VERSION_V2 = "analytical-visualizations.v2"
# Project Spec S0227: the v3 (continuous-regression) INTERNAL Atlas-native
# fixed-configuration training-record profile -- mirrors the v2 (multiclass)
# recognition above exactly: still an internal M24 candidate (never coerced
# to manual_governed_input), never falls through to the legacy v1 governance
# constants, and requires its own real declared training-metrics.v3 to be
# carried forward verbatim rather than any other metrics version. The
# governed visualization role for a v3 candidate is reserved as
# analytical-visualizations.v3 -- a real v3 visualization artifact/schema is
# not implemented by this spec, so a v3 provenance candidate can only ever
# be built once that future artifact exists; a caller that instead points
# the visualizations role at a v1/v2 classification visualization is
# rejected as a mismatch (fails closed), never silently substituted.
...
def _resolve_training_provenance(
    ...
    metrics_version_override, visualizations_version_override) provenance
    tuple for this candidate input.
    ...
    metadata). Project Spec S0216 additionally recognizes the real internal
    Atlas-native multiclass training-parameter-record.v2 profile: still the
    internal M24 source_stage (never manual_governed_input), but with its
    own real training-metrics.v2/analytical-visualizations.v2 versions
    carried forward instead of the fixed v1 governance constants -- never
    coerced to v1. Every other value -- the real internal
    ...
    path and returns (None, None, None) overrides, so _build_artifact_input
    falls through to its pre-existing fixed governance values exactly as
    before S0180/S0216 (which never inspected this file's content for these
    roles at all). Once external or native-v2 provenance is confirmed,
    training_metrics gets cross-checked and required to agree: a v1 training
    record pairs only with v1 metrics, a v2 (multiclass) training record
    only with v2 metrics -- never mixed; a mismatch raises ValueError, the
    same failure convention this function's caller already uses.
    """
    ...
    if record_version == _TRAINING_PARAMETER_RECORD_INTERNAL_VERSION_V2:
        ...
        if metrics_version != _TRAINING_METRICS_INTERNAL_VERSION_V2:
            raise ValueError(
                "training_metrics contract_version does not agree with training_parameter_record "
                f"provenance: expected {_TRAINING_METRICS_INTERNAL_VERSION_V2!r}, got {metrics_version!r}"
            )
        ...
        if visualizations_version != _ANALYTICAL_VISUALIZATIONS_INTERNAL_VERSION_V2:
            raise ValueError(
                "visualizations contract_version does not agree with training_parameter_record "
                f"provenance: expected {_ANALYTICAL_VISUALIZATIONS_INTERNAL_VERSION_V2!r}, got "
                f"{visualizations_version!r}"
            )
        return "M24", False, record_version, metrics_version, visualizations_version

    if record_version == _TRAINING_PARAMETER_RECORD_INTERNAL_VERSION_V3:
        training_metrics_path = repo_root / artifact_references["training_metrics"]
        metrics_version = _read_declared_contract_version(training_metrics_path)
        if metrics_version != _TRAINING_METRICS_INTERNAL_VERSION_V3:
            raise ValueError(
                "training_metrics contract_version does not agree with training_parameter_record "
                f"provenance: expected {_TRAINING_METRICS_INTERNAL_VERSION_V3!r}, got {metrics_version!r}"
            )
        visualizations_path = repo_root / artifact_references["visualizations"]
        visualizations_version = _read_declared_contract_version(visualizations_path)
        if visualizations_version != _ANALYTICAL_VISUALIZATIONS_INTERNAL_VERSION_V3:
            raise ValueError(
                "visualizations contract_version does not agree with training_parameter_record "
    ...
    if record_version not in (
        ...
        return "M24", False, None, None, None
    ...
    return _EXTERNAL_MODEL_SOURCE_STAGE, True, record_version, metrics_version, None
    ...
    *,
    ...
    artifact_inputs = {
        ...
        "model_card": _build_artifact_input(
            "model_card",
            artifact_references["model_card"],
            resolved_repo_root,
            source_stage=training_provenance_stage,
        ),
    ...
    artifact_inputs["visualizations"] = _build_artifact_input(
        "visualizations",
        artifact_references["visualizations"],
        resolved_repo_root,
        # Project Spec S0216: only the native-v2 provenance branch supplies
        # a visualizations_version override (carrying the real
        # analytical-visualizations.v2 declaration forward instead of the
        # fixed v1 governance constant below); every other provenance
        # (historical internal v1, or external S0157/S0208) is unaffected
        # and keeps its pre-existing fixed "M24"/analytical-visualizations.v1
        # governance behavior exactly as before S0216.
        contract_version=visualizations_version,
    )
...
RELEASE_LAYER_SUPPORTED_CAPABILITY_PROFILE_IDS = frozenset({
    ...
    # Project Spec S0227: recognized architecture-level identity only. The
    # real committed continuous-predictive-regression.v1 capability profile
