"""
Project Spec S0206: Multiclass Predictive Classification Capability and
...
DATASET_SLUG = "sample-multiclass-classification-dataset"

DISCOVERY_EVIDENCE_BYTES = b'{"schema_version": "dataset-discovery-evidence.v1"}'
SEMANTIC_INTENT_BYTES = b'{"schema_version": "dataset-semantic-intent.v2"}'
...
def _load_profile_text() -> str:
    return MULTICLASS_PROFILE_PATH.read_text(encoding="utf-8")
...
class TestProfileFileExistsAndParses:
    def test_profile_file_exists(self):
        assert MULTICLASS_PROFILE_PATH.is_file()
...
class TestIdentityVersionSupportStatus:
    ...
    def test_capability_profile_id(self):
        assert _load_profile()["capability_profile_id"] == "multiclass-predictive-classification"
...
class TestSemanticRuntimePublicationPolicy:
    ...
    def test_prediction_runtime_mode(self):
        profile = _load_profile()
        assert profile["prediction_runtime"]["mode"] == "single_model_multiclass_classification"
...
class TestNoDatasetOrTelcoOrDryBeanSpecificCoupling:
    def test_no_dataset_slug_or_dataset_name_in_profile_text(self):
        text = _load_profile_text().lower()
        assert "sample-multiclass-classification-dataset" not in text
        assert "dataset_slug" not in text

    def test_no_telco_or_dry_bean_specific_identity_in_profile_text(self):
        text = _load_profile_text().lower()
        assert "telco" not in text
        assert "churn" not in text
        assert "tenure" not in text
        assert "dry bean" not in text
        assert "dry_bean" not in text
        assert "drybean" not in text
        for bean_class_name in ("seker", "barbunya", "bombay", "cali", "dermason", "horoz", "sira"):
            assert bean_class_name not in text
...
class TestBinaryProfileRemainsCurrentOperationalProfile:
...
class TestContractDerivationAndReleaseConstantsRemainBinary:
    def test_contract_derivation_currently_supported_capability_profile_id_is_binary(self):
        assert CURRENTLY_SUPPORTED_CAPABILITY_PROFILE_ID == "binary-predictive-classification"

    def test_assemble_candidate_currently_supported_release_capability_profile_id_is_binary(self):
        assert CURRENTLY_SUPPORTED_RELEASE_CAPABILITY_PROFILE_ID == "binary-predictive-classification"

    def test_multiclass_profile_id_does_not_match_either_currently_supported_constant(self):
        profile_id = _load_profile()["capability_profile_id"]
        assert profile_id != CURRENTLY_SUPPORTED_CAPABILITY_PROFILE_ID
...
class TestAuthoringBoundaryCompatibility:
    """Proves the real multiclass profile file participates in a synthetic
    S0166/S0206 authoring-boundary validation (pipeline.authoring_contracts)
    using only inert fixture bytes under tmp_path -- never a real model,
    notebook, or release artifact."""
