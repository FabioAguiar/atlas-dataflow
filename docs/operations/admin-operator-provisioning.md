...
| --- | --- | --- |
| Trusted (backend only) | `ATLAS_SUPABASE_JWT_ISSUER`, `ATLAS_SUPABASE_JWT_AUDIENCE`, `ATLAS_SUPABASE_JWKS_URL`, `ATLAS_ADMIN_USER_ID` | Private `api` service environment |
...
1. In the hosted Supabase project, create the single operator user.
...
4. Disable email signup in the project so no other account can be created.
5. Supply the six values above to the private deployment only, then rebuild the private web image so the publishable values are inlined.
...
- Hosted-project checks require operator-supplied access; when unavailable they are recorded as blocked, not passed.
