...
def test_inference_result_dispatch_accepts_both_governed_families():
...
@pytest.mark.parametrize(
    ("result", "expected"),
    [(MULTICLASS_RESULT, BINARY_EXPECTED), (BINARY_RESULT, MULTICLASS_EXPECTED)],
)
def test_inference_result_dispatch_rejects_individually_valid_wrong_family(result, expected):
...
@pytest.mark.parametrize(
    ...
    [
        None,
        {},
        {**BINARY_EXPECTED, "schema_version": "unknown.v1"},
        {**BINARY_EXPECTED, "problem_type": "multiclass_classification"},
        {**BINARY_EXPECTED, "result_schema_version": "multiclass-classification-result.v1"},
    ],
)
def test_inference_result_dispatch_rejects_missing_unknown_or_mixed_semantics(expected):
    _assert_rejected(BINARY_RESULT, expected)


def test_inference_result_dispatch_uses_only_the_selected_strict_validator(monkeypatch):
    calls = []
    ...
    assert calls == ["binary", "multiclass"]


@pytest.mark.parametrize(
    ("result", "expected"),
    [({**BINARY_RESULT, "unexpected": True}, BINARY_EXPECTED), ({**MULTICLASS_RESULT, "unexpected": True}, MULTICLASS_EXPECTED)],
)
def test_inference_result_dispatch_rejects_malformed_selected_family(result, expected):
    _assert_rejected(result, expected)
